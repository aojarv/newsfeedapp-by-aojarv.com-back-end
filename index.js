  ...
  const getTimesWire = fetch(
    ...
    .then(json => {
      for (let i = 0; i < json.results.length; i++) {
        ...
        let cpr = ``;
        if (
          !!json.results[i].multimedia &&
          json.results[i].multimedia.length > 0
        ) {
...
const getAllTopstoriesData = async () => {
  ...
  for (let j = 0; j < sections.length; j++) {
    ...
    for (let i = 0; i < dataFromUrl.results.length; i++) {
      ...
      let cpr = "";
      if (
        !!dataFromUrl.results[i].multimedia &&
        dataFromUrl.results[i].multimedia.length > 0
      ) {
